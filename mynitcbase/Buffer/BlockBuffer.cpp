...
{
    // if blockNum is INVALID_BLOCK or unused 
	if (blockNum == INVALID_BLOCKNUM || StaticBuffer::blockAllocMap[blockNum] == UNUSED_BLK)
		return;

	// get the buffer number of the block 
	int bufferIndex = StaticBuffer::getBufferNum(blockNum);

	// free the buffer by setting the free field of the corresponding BufferMetaInfo entry to true
	if (bufferIndex >= 0 && bufferIndex < BUFFER_CAPACITY)
		StaticBuffer::metainfo[bufferIndex].free = true;

	// free the block in disk by setting the blockAllocMap entry corresponding to blockNum to UNUSED_BLK
